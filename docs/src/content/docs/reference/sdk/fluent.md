...
description: Calling the same domain functions from the thing you are holding.
---

## Fluent

Methods on the thing you are holding. Each forwards to the domain function of the same name — same capability, same behaviour. The prelude imports them.
...
What each group hands back. `player.punish()` gives a `Punish`, and the verbs live inside it.
