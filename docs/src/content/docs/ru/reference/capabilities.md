...
## Та, что читает чужую переписку

`dm = ["read"]` открывает личные сообщения — то, что двое написали друг другу, на сайте
или в игре. Политика приватности инстанса обещает, что персонал не читает их в порядке
модерации; модуль с этой возможностью — то единственное исключение, на которое оператор
согласился сам.

Просите её, когда она нужна делу: фильтр ссылок, автоответчик для того, кого нет, бот,
который отвечает. Не просите «на всякий случай» — оператор видит эту строку в вашем
манифесте и должен принять решение, а модуль, который читает переписку без надобности,
— это модуль, которому стоит отказать.

`dm = ["send"]` пишет от имени игрока, и получатель видит его имя. Это намеренно: он
будет отвечать, а сообщение «ни от кого» оставляет его без того, кому ответить. В истории
записано, какой модуль это сделал, и в переписке это видно.
