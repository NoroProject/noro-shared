...
impl Registration {
    ...
    pub fn violations(&self) -> Vec<String> {
        ...
        for e in &self.events {
            ...
            // значится: каталог — это события мастера.
            //
            // Владелец здесь не проверяется, и не должен: подписка на чужое
            // событие — поддерживаемая форма межмодульной интеграции, см.
            // `#[event("mod.shop.purchase")]` в `noro_sdk`. Подделать **выпуск**
            // чужого события нельзя — имя там собирает мастер из идентификатора
            // самого модуля.
