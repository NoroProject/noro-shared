...
/// Действия минюста над городами.
///
/// Дробно, а не одним «управлять»: одобрить заявку, перекроить чужие границы и
/// распустить город — это разные доверия. Сотруднику, который принимает
/// заявки, незачем уметь стирать чужую территорию, а тому, кто ставит метки на
/// карте, — распускать города.
///
/// Ветка выдаётся и целиком: `noro.hub.<server>.town.*` — это «весь минюст».
pub const TOWN_APPROVE: &str = "approve";
pub const TOWN_CREATE: &str = "create";
pub const TOWN_EDIT: &str = "edit";
pub const TOWN_LAND: &str = "land";
pub const TOWN_LIMIT: &str = "limit";
pub const TOWN_DISSOLVE: &str = "dissolve";
pub const TOWN_MARK: &str = "mark";

/// Узел, которым минюст выдавался до дробления прав.
///
/// Держится ради уже выданных ролей: у кого он есть, тот умеет всё, что умел
/// раньше. Новым ролям выдают отдельные действия или ветку целиком.
pub const TOWN_LEGACY_MANAGE: &str = "manage";

/// Все действия минюста. По нему собирается подсказка прав и решается,
/// показывать ли вкладку вообще.
pub const TOWN_ACTIONS: &[&str] = &[
    TOWN_APPROVE,
    TOWN_CREATE,
    TOWN_EDIT,
    TOWN_LAND,
...
/// Действия в суде подсайта.
///
/// `judge` — брать дела, вести слушание, писать решение и видеть совещательную
/// комнату. Подавать иск отдельного права не требует: это возможность
/// участника, а не привилегия, и ограничивает её пошлина, а не узел.
pub const COURT_JUDGE: &str = "judge";

pub const COURT_ACTIONS: &[&str] = &[COURT_JUDGE];

/// Право на действие в суде подсайта.
pub fn perm_hub_court(server_id: &str, action: &str) -> String {
...
/// казначея назначает сам город строкой в `hub_town_members`. Смешать их
/// значило бы звать администрацию к каждому назначению в каждой мэрии.
pub fn perm_hub_town(server_id: &str, action: &str) -> String {
    format!("noro.hub.{server_id}.town.{action}")
}
