...
pub enum ClientWsMsg {
    ...
    /// Кадр форку лаунчера в его модуль на мастере.
    ///
    /// Форма содержимого — дело пары «форк и его модуль»: протокол её не знает
    /// и знать не должен, иначе каждая чужая доработка требовала бы релиза
    /// `schema`. Ровно поэтому здесь `Value`, а не типизированные поля.
    ///
    /// Обратная сторона — ответа на этот кадр нет. Где нужен ответ, форк зовёт
    /// ручку модуля по HTTP: там есть и код ошибки, и тело.
    ModuleMessage {
        /// Идентификатор модуля-получателя.
        module: String,
...
pub enum ServerWsMsg {
    ...
    /// Кадр модуля форку лаунчера. Зеркало `ClientWsMsg::ModuleMessage`.
    ///
    /// Стоковый лаунчер его не разбирает и молча пропускает — разбор кадров
    /// устроен так, что неизвестный вариант ничего не ломает. Это и есть
    /// условие, при котором доработку можно ставить на часть машин.
    ModuleMessage {
        module: String,
        payload: serde_json::Value,
    },
